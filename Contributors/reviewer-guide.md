...
Each Pull Request automatically generates a **live preview** of the site with the new content included. Look for a comment on the PR from GitHub Actions — it will contain a link. Click that link to see exactly how the content will look on `math.ctri.ng`, with rendered equations, working links, and the graph view.

This is the best way to review — you see exactly what the student will see.

> If no preview link appears, you can still review by reading the raw markdown in the **Files changed** tab. Equations are written between `$...$` for inline and `$$...$$` for display.
...
- [ ] Does the LaTeX render correctly in the preview?
